...
async function reconcileSections(adminId, powerBiSections, databaseSections) {
  ...
  const sectionsAreEqual =
    databaseSectionsIds.every((databaseSectionId) => powerBiSectionsIds.includes(databaseSectionId)) && !databaseSectionsIds.length === 0;

  if (!sectionsAreEqual) {
    powerBiSections.forEach(async (powerBiSection) => {
      if (!databaseSectionsIds.includes(stringifySectionKey(powerBiSection)))
        await sectionsRepository.createSectionByAdmin(adminId, powerBiSection.id, powerBiSection.reportId);
    });

    databaseSections.forEach(async (databaseSection) => {
      if (!powerBiSectionsIds.includes(stringifySectionKey(databaseSection))) {
        await sectionsRepository.deleteSectionByAdmin(adminId, databaseSection.sectionId, databaseSection.reportId);
      }
    });
  }
}
...
exports.readSectionsByAdmin = async (adminId) => {
  ...
  const powerBiSections = await sectionsPowerBiRepository.readSectionsByManyReports(reports);
  ...
  const sections = databaseSections.map((databaseSection) => ({
    ...databaseSection,
    ...powerBiSections.find((powerBiSection) => powerBiSection.id === databaseSection.sectionId && powerBiSection.reportId === databaseSection.reportId),
    sectionId: undefined,
  }));
...
exports.readSectionsByReport = async (workspaceId, reportId) => {
  return await sectionsPowerBiRepository.readSectionsByReport(workspaceId, reportId);
};
...
exports.readSectionsByUser = async (userId, adminId) => {
  ...
  const databaseUserSections = await sectionsRepository.readSectionsByUser(userId);

  const sections = databaseUserSections.map((databaseUserSection) => ({
    ...databaseUserSection,
    ...adminSections.find((section) => section.id === databaseUserSection.sectionId && section.reportId === databaseUserSection.reportId),
  }));
