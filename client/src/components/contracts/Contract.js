...
const Contract = () => {
  ...
  const { identificationDocuments } = useSelector((state) => state.identificationDocuments);
  ...
  if (!contract || regions.length === 0 || countries.length === 0 || zones.length === 0) return <CircularLoading />;

  const thisContractCurrency = currencies.find((currency) => currency.id === contract.currencyId);

  const thisCountryRegions = regions.filter((region) => region.countryId === contractFields.countryId);
  const thisRegionZones = zones.filter((zone) => zone.regionId === contractFields.regionId);

  const thisCountryIdentificationDocument = identificationDocuments.find(
    (identificationDocument) => identificationDocument.countryId === contractFields.countryId
  );
      ...
      <Grid container spacing={matchMd ? 8 : 3}>
        <Grid lg={5} md={12} item>
          <Grid container alignItems="center">
            <FormField label="País">
              <FormField.Select {...bindField("countryId")} options={countries} optionValue="id" display="name" />
            </FormField>

            <FormField label={thisCountryIdentificationDocument?.type}>
              <FormField.TextField {...bindField("identificationDocumentValue")} />
            </FormField>
            ...
            <FormField label="Región">
              <FormField.Select {...bindField("regionId")} options={thisCountryRegions} optionValue="id" display="name" />
            </FormField>

            <FormField label="Comuna">
              <FormField.Select {...bindField("zoneId")} options={thisRegionZones} optionValue="id" display="name" />
            </FormField>
          </Grid>
        </Grid>
        ...
        <Grid item lg={7} md={12} mt={matchMd ? 2 : 0}>
          <Grid container spacing={matchMd ? 3 : 2}>
            ...
            <InformationItem label="Moneda">{thisContractCurrency?.code}</InformationItem>
          </Grid>
        </Grid>
      </Grid>
