...
import { readReportGroupsByAdmin, readReportsByAdmin, updateReportsGroup, createReportsGroup } from "../../state/reports/reportsActions";
...
const ManageReportsGroup = () => {
  ...
  const { reportsGroups, reports, loading: reportsLoading } = useSelector((state) => state.reports);
  ...
  const buttonHasBeenClicked = useNavigateAfterAction(reportsLoading, "/admins/reports-groups");
  ...
  if (reportsGroupId) {
    thisReportsGroup = reportsGroups.find((reportsGroup) => reportsGroup.id === parseInt(reportsGroupId));
  }

  const [active, handleSwitchChange] = useToggle(true);

  const [selectedSections, toggleSelectedSection, setSelectedSections] = useSelectionList([]);
  ...
  const thisWorkspaceReports = reports.filter((report) => report.workspaceId === dropdowns.workspaceId);

  const thisReportSections = sections.filter((section) => section.reportId === dropdowns.reportId);
  ...
  const handleManageReportsGroup = () => {
    ...
    dispatch(reportsGroupId ? updateReportsGroup(reportsGroupData) : createReportsGroup(reportsGroupData));

    buttonHasBeenClicked();
  };

  const handleSectionSelection = (e) => {
    setSelectedSections(typeof e.target.value === "string" ? e.target.value.split(",") : e.target.value);
  };
  ...
  useEffect(() => {
    if (thisWorkspaceReports.length > 0) setDropdowns({ ...dropdowns, reportId: thisWorkspaceReports[0].id });
  ...
  return (
    <Paper className="container">
      ...
      <ManageReportsGroupTable sections={selectedSections} onChange={toggleSelectedSection} />

      <Grid mt={4} container justifyContent="space-between">
        <Button onClick={() => navigate("/admins/reports-groups")}>Cancelar</Button>
